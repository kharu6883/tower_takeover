...
void opcontrol() {
	...
	macro::Slew rackSlew(30, 60, true); // Accel, Decel
	...
	while (true) {
		...
		if(master.get_digital(DIGITAL_L1) && !master.get_digital(DIGITAL_L2)) {
			...
			if(!isTrack) { // Put up Rack
				...
				rackPID.withConst(0.13).calculate(RACK_UP, rack.getPot());
		...
		/*--------------------------------
				ARM CONTROL
		--------------------------------*/
		if(master.get_digital_new_press(DIGITAL_DOWN)) arm.reset();

		if(master.get_digital(DIGITAL_R1) && master.get_digital(DIGITAL_R2)) towerMode = 0;
		if(master.get_digital(DIGITAL_L1) && master.get_digital(DIGITAL_L2)) towerMode = 1;
		...
		switch(towerMode) {
			...
			default: arm.move(ARM_BOTTOM, 127);
		}
		...
		if(towerMode == 0 || towerMode == 4 || towerMode == 5 || towerMode == 6 || towerMode == 420) io::roller(roller.getOutput());
